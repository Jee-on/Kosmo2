...
<%
	...
	DeleteMessageService dao = DeleteMessageService.getInstance();
	delCnt =dao.delete(mId,password);
...
<body>
	<% if( delCnt == 0 ) { pageContext.forward("listMessage.jsp"); %>
		삭제할 메세지가 존재하지 않거나 비밀번호가 올바르지 않습니다.
		
	<% } else { pageContext.forward("listMessage.jsp"); %>
		메세지를 삭제하였습니다.
